...
- **Run-once exit:** after the retry budget is exhausted (or on a fatal `4xx`), the run emits the ❌ notification and the process **exits non-zero** so the Job is marked Failed and is visible in `kubectl get jobs` / alerting.
- Implementation: Spring Framework 7's **native** retry — `org.springframework.core.retry.RetryTemplate` + `RetryPolicy`, invoked **imperatively** in `TiamatNetexPublisher.publish` (`retryTemplate.execute(...)`); `RestClient` has no built-in retry. This is *not* the legacy Spring Retry library, and the annotation-based `@Retryable` is a future enhancement only (see the TODO in `PapsukkalApplication`). Transient-vs-fatal classification lives in `TransientHttpErrorPredicate`: `HttpServerErrorException` (`5xx`), `ResourceAccessException` (IO/timeout) and HTTP `429` are transient; other `4xx` (`HttpClientErrorException`) are fatal — note `429` arrives as an `HttpClientErrorException` subtype, so it needs the explicit status check rather than a blanket "retry 5xx / fail 4xx" rule. The two retry layers are distinct: the in-run `RetryTemplate` handles *within-run* transient blips; the daily CronJob handles *across-run* recovery.
